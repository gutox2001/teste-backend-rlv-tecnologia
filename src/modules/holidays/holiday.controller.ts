...
export class HolidayController {
    ...
    @Post('')
    @ApiOperation({ summary: 'Cria feriados nacionais' })
    @ApiResponse({ status: 201, description: 'Feriados nacionais criados com sucesso' })
    @ApiResponse({ status: 400, description: 'Erro ao criar feriados nacionais' })
    async createNacionalHolidays(
        @Body('nacionalHolidays') nacionalHolidays: ICreateHoliday[],
    ) {
    ...
    async updateHolidays(
        ...
        const result = await this.putHolidayService.execute(ibge_code, date, name);
