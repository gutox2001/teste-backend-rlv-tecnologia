...
export class PutHolidayService {
    ...
    async execute(ibge_code: string, date: string, name: string): Promise<{ name: string, created: boolean }> {
        ...
        if (nationalHoliday) {
            return { name: nationalHoliday.name, created: false };
        } else {
            ...
            switch (ibge_code.length) {
                ...
                case 2:
                    ...
                    const stateHoliday = (await this.holidaysRepository.findByStateIbgeCode(ibge_code)).find(holiday => holiday.date === date);
                    ...
                    if (!stateHoliday) {
                        ...
                        return { name: stateHoliday.name, created: false };
                    }
                ...
                case 7:
                    ...
                    const cityHoliday = (await this.holidaysRepository.findByCityIbgeCode(ibge_code)).find(holiday => holiday.date === date);
                    ...
                    if (!cityHoliday) {
                        ...
                        await this.holidaysRepository.update(cityHoliday.id, {
                            ...
                            type: "MUNICIPAL",
                            stateId: cityHoliday.state.id,
                        });

                        return { name: cityHoliday.name, created: false };
                    }
